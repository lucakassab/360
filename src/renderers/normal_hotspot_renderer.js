...
function applyRootOrientation(rootEl, hotspot, hotspotRotationYawOffsetDeg = 0) {
  ...
  rootEl.setAttribute(
    ...
    rotToString({
      ...
      y: num(hotspot.rotation?.y, 0) + num(hotspotRotationYawOffsetDeg, 0),
...
function createCollider(iconSize) {
  ...
  const colliderSize = iconSize * 1.25;

  collider.classList.add("clickable-hotspot");
...
function attachHotspotEvents(targetEl, rootEl, hotspot, onHotspotNavigate) {
  ...
  targetEl.addEventListener("mouseleave", () => {
    console.log("[hotspot] mouseleave", {
      id: hotspot.id,
      label: hotspot.label,
    });
  ...
  targetEl.addEventListener("click", async (event) => {
    console.log("[hotspot] click recebido", {
      id: hotspot.id,
      label: hotspot.label,
      targetScene: hotspot.targetScene,
      targetTour: hotspot.targetTour,
    ...
    if (navigating || typeof onHotspotNavigate !== "function") {
      console.warn("[hotspot] click ignorado", {
        id: hotspot.id,
        navigating,
        hasHandler: typeof onHotspotNavigate === "function",
      });
      return;
    }
    ...
    try {
      console.log("[hotspot] iniciando navegação", {
        id: hotspot.id,
        label: hotspot.label,
        targetScene: hotspot.targetScene,
        targetTour: hotspot.targetTour,
      ...
      console.log("[hotspot] navegação concluída", {
        id: hotspot.id,
        label: hotspot.label,
      });
    } catch (error) {
      console.error("[hotspot] erro ao navegar", {
        id: hotspot.id,
        label: hotspot.label,
        error,
      });
...
function createHotspotEntity({
  ...
  rootEl.id = `hotspot-${sanitizeId(hotspot.id || `item-${index}`)}`;
  rootEl.dataset.hotspotId = hotspot.id || `item-${index}`;

  rootEl.setAttribute("position", vecToString(hotspot.pos));
  applyRootOrientation(rootEl, hotspot, hotspotRotationYawOffsetDeg);
...
export function renderNormalHotspots({
  sceneEl,
  sceneData,
  tourData,
  tourPath,
